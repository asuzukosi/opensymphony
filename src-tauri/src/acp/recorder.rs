...
mod tests {
    ...
    #[test]
    fn tracks_message_segments_and_tool_lifecycle() {
        let mut recorder = Recorder::new();
        let mut kinds = Vec::new();
        let notify = |update: SessionUpdate| {
            SessionNotification::new(SessionId::new("agent-session-1"), update)
        };

        recorder.handle_update(
            &notify(SessionUpdate::AgentMessageChunk(ContentChunk::new(
                ContentBlock::Text(TextContent::new("starting work")),
            ))),
            &mut |kind, _| kinds.push(kind),
        );
        recorder.handle_update(
        ...
        recorder.handle_update(
            &notify(SessionUpdate::AgentMessageChunk(ContentChunk::new(
                ContentBlock::Text(TextContent::new("done for now")),
            ))),
            &mut |kind, _| kinds.push(kind),
        );

        assert_eq!(recorder.messages.completed, vec!["starting work"]);
        assert!(recorder.tools_in_progress.contains_key(&ToolCallId::new("tool-1")));
        assert_eq!(recorder.last_agent_message(), Some("done for now".into()));
        ...
        assert!(recorder.tools_in_progress.is_empty());
        assert_eq!(recorder.tools_completed.len(), 1);
        assert!(kinds.contains(&SessionEventKind::ToolCall));
        assert!(kinds.contains(&SessionEventKind::SessionUpdate));
    }
}
