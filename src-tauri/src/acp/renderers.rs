...
mod tests {
    use super::{
        render_prompt_template, PromptRenderError, PromptRenderIssueFields, RenderPromptInput,
    };

    #[test]
    fn substitutes_allowed_placeholders() {
        let issue = PromptRenderIssueFields {
            identifier: "sym-42".into(),
            title: "Add login form".into(),
            description: Some("Build the auth UI.".into()),
        };

        let rendered = render_prompt_template(&RenderPromptInput {
            prompt_template: "{{identifier}}: {{title}}\n{{description}}",
            issue: &issue,
        })
        .expect("render prompt");

        assert_eq!(rendered, "sym-42: Add login form\nBuild the auth UI.");

        let no_description = render_prompt_template(&RenderPromptInput {
        ...
        assert_eq!(no_description, "");

        let err = render_prompt_template(&RenderPromptInput {
            prompt_template: "Hello {{unknown}}",
            issue: &PromptRenderIssueFields {
                identifier: "sym-42".into(),
                title: "Add login form".into(),
                description: None,
