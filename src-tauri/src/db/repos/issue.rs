...
use crate::types::{
    BoardColumnId, Issue, IssueHeader, IssuePatch, Platform, ProjectBoardIssue,
...
impl<'a> IssueRepo<'a> {
    ...
    pub fn list_by_project(&self, project_id: &str) -> DbResult<Vec<ProjectIssueListItem>> {
        ...
        while let Some(row) = rows.next()? {
            items.push(map_issue_list_item(row)?);
        }
        Ok(items)
    }
    ...
    pub fn set_auto_approve_permissions(&self, id: &str, auto_approve: bool) -> DbResult<Issue> {
        let changed = self.conn.execute(
            "UPDATE issues SET auto_approve_permissions = ?1, updated_at = datetime('now') WHERE id = ?2",
