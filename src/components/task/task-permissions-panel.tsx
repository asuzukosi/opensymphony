...
import { TaskErrorAlert } from "@/components/task/task-states";
...
function taskHasActiveSession(attempts: TaskDetailRunAttempt[]): boolean {
...
export function TaskPermissionsPanel({ taskId, attempts }: TaskPermissionsPanelProps) {
  const hasActiveSession = useMemo(() => taskHasActiveSession(attempts), [attempts]);
  ...
  const {
    ...
    resolveError,
    resetResolve,
  } = useTaskPermissions({ taskId, pollIntervalMs });
  ...
  if (error) {
    return <TaskErrorAlert error={error} />;
  }
    ...
    <Alert className="border-amber-500/40 bg-amber-500/5 text-xs">
      ...
      <AlertDescription className="space-y-3">
        <p className="text-muted-foreground">
          Agents are blocked until you approve or deny each permission request for this task.
        </p>
